...
const TEAM_ACCENT = {
  ...
  "Phoenix Suns": "#E56020",
  "Portland Trail Blazers": "#FFFFFF",
  "Sacramento Kings": "#63727A",
  "San Antonio Spurs": "#C4CED4",
  "Toronto Raptors": "#000000",
  "Utah Jazz": "#F9A01B",
...
// Coach championship rings won as head coach
// Key: "Coach Name|Franchise" for cases where same coach won with different teams
const COACH_CHAMPS = {
  "R. Auerbach": 9, // Boston 1957–66
  "B. Russell": 2, // Boston 1968–69
  "T. Heinsohn": 2, // Boston 1974, 1976 — wait, Heinsohn won 1974
  "B. Fitch": 1, // Boston 1981
  "K. Jones": 2, // Boston 1984, 1986
  "P. Riley": 5, // LA Lakers ×4, Miami ×1
  "P. Jackson": 11, // Chicago ×6, LA Lakers ×5
  "C. Daly": 2, // Detroit 1989–90
  "L. Costello": 1, // Milwaukee 1971
  "D. Nelson": 0,
  "A. Hannum": 2, // St. Louis 1958, Philadelphia 1967
  "B. Sharman": 1, // LA Lakers 1972
  "J. Kundla": 5, // Minneapolis Lakers
  "R. Tomjanovich": 2, // Houston 1994–95
  "G. Popovich": 5, // San Antonio
  "L. Brown": 1, // Detroit 2004
  "E. Spoelstra": 3, // Miami 2012, 2013, 2020
  "S. Kerr": 4, // Golden State 2015, 2017, 2018, 2022
  "T. Lue": 1, // Cleveland 2016
  "B. Hill": 0,
  "S. Van Gundy": 1, // Miami 2006
  "F. Vogel": 1, // LA Lakers 2020
  "N. Nurse": 1, // Toronto 2019
  "R. Carlisle": 1, // Dallas 2011
  "J. Mazzulla": 1, // Boston 2024
  "I. Udoka": 0,
  "D. Casey": 0,
  "B. Stevens": 0,
  "D. Rivers": 0,
  "M. Budenholzer": 1, // Milwaukee 2021
  "M. Malone": 1, // Denver 2023
  "B. Cunningham": 1, // Philadelphia 1983
  "A. Attles": 1, // Golden State 1975
  "J. Ramsay": 1, // Portland 1977
  "L. Harrison": 1, // Rochester Royals 1951
  "E. Gottlieb": 1, // Philadelphia Warriors 1956
  "J. Kundla": 5,
  "T. Heinsohn": 2,
  "D. Moe": 0,
  "K. Loughery": 0,
...
function coachChamps(coach, franchise, start, end) {
  // Special cases where same coach name won with different teams
  if (coach === "P. Riley") {
    const lakersRings = franchise === "Los Angeles Lakers" ? 4 : 0;
    const miamiRings = franchise === "Miami Heat" && end > 2006 ? 1 : 0;
  ...
  if (coach === "A. Hannum") {
    // Won 1958 with Hawks, 1967 with Philadelphia
...
// Franchise historical stats sourced from nba_all_win_percent.xlsx + PlayoffAppearancesByTeam.xlsx
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            zIndex: 2,
            transition:
              dragging === "start" ? "none" : "left 0.05s, background 0.15s",
          }}
        />
        <div
          ...
          style={{
            ...
            zIndex: 2,
            transition:
              dragging === "end" ? "none" : "left 0.05s, background 0.15s",
          }}
        />
      </div>
      <span
...
export default function App() {
  ...
  const sortedFranchises = [...FRANCHISES].sort((a, b) => {
    ...
    if (sortMode === "coaches") {
      return (
        RAW_DATA.filter((d) => d.franchise === b).length -
        RAW_DATA.filter((d) => d.franchise === a).length
      );
    }
    if (sortMode === "longestTenure") {
      const longest = (f) =>
        Math.max(
          ...RAW_DATA.filter((d) => d.franchise === f).map(
            (d) => d.end - d.start
          )
        );
  ...
  const filteredFranchises = sortedFranchises.filter(
    (f) =>
      f.toLowerCase().includes(search.toLowerCase()) ||
      RAW_DATA.some(
        (d) =>
          d.franchise === f &&
          d.coach.toLowerCase().includes(search.toLowerCase())
      )
  );
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <div
        style={{ padding: "24px 32px 16px", borderBottom: "2px solid #ebebeb" }}
      >
        <div style={{ display: "flex", alignItems: "baseline", gap: 16 }}>
          <h1
            style={{
              ...
              letterSpacing: "-0.5px",
              background:
                "linear-gradient(90deg, #1a1a2e 0%, #c0392b 50%, #e67e22 100%)",
          ...
          <span style={{ color: "#aaa", fontSize: 13 }}>
            1946 – 2026 · {RAW_DATA.length} coaching stints
          </span>
        </div>
        <p
          style={{
      ...
      >
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          ...
          {[
            ["champ", "🏆 Championships"],
            ["alpha", "A–Z"],
            ["coaches", "Most Coaches"],
            ["longestTenure", "Longest Tenure"],
          ].map(([k, l]) => (
            <button
              key={k}
              className={`ctrl-btn ${sortMode === k ? "active" : ""}`}
        ...
        >
          ...
          {[
            [MIN_YEAR, MAX_YEAR, "All Time"],
            [1980, MAX_YEAR, "Since 1980"],
            [2000, MAX_YEAR, "Since 2000"],
          ].map(([s, e, l]) => (
            <button
              key={l}
              className={`ctrl-btn ${
                viewStart === s && viewEnd === e ? "active" : ""
              }`}
              onClick={() => {
                setViewStart(s);
                setViewEnd(e);
        ...
        {selectedFranchise && (
          <button
            className="ctrl-btn"
            style={{ borderColor: "#E03A3E", color: "#c0392b" }}
      ...
      >
        ...
        >
          ...
          {filteredFranchises.map((franchise, fi) => {
            ...
            const teamBlocks = RAW_DATA.filter(
              (d) =>
            ...
            return (
              <g
                ...
                onClick={() =>
                  setSelectedFranchise(isSelected ? null : franchise)
                }
              >
                ...
                <text
                  ...
                  onMouseEnter={(e) => {
                    const allBlocks = RAW_DATA.filter(
                      (d) => d.franchise === franchise
                    );
                    const numCoaches = new Set(allBlocks.map((d) => d.coach))
                      .size;
                    setFranchiseTip({
                      franchise,
                      numCoaches,
                ...
                >
                  ...
                  {sortMode === "champ" && (CHAMPIONSHIPS[franchise] ?? 0) > 0
                    ? `  🏆${CHAMPIONSHIPS[franchise]}`
                    : ""}
                </text>
                ...
                {teamBlocks.map((block, bi) => {
                  ...
                  // Color intensity based on win pct
                    ...
                    >
                      ...
                      {w > 30 && (
                        ...
                        >
                          {w > 60
                            ? block.coach
                            : block.coach.split(" ")[1] || block.coach}
                        </text>
                      )}
                    </g>
                  );
      ...
      {franchiseTip &&
        (() => {
          ...
          const tx = franchiseTip.x + 14;
          const ty = franchiseTip.y - 10;
          return (
            <div
              style={{
                position: "fixed",
            ...
            >
              ...
              >
                ...
                <div
                  style={{
                    fontWeight: 800,
                    color: fs.champs > 0 ? "#b8860b" : "#bbb",
                    fontSize: 14,
                  }}
                >
                  {fs.champs > 0
                    ? "🏆".repeat(Math.min(fs.champs, 10)) +
                      (fs.champs > 10 ? ` ×${fs.champs}` : "")
                    : "—"}
                </div>
              </div>
            </div>
      ...
      {tooltip && (
        <div
          style={{
            ...
            background: "#ffffff",
            border: `1.5px solid ${
              TEAM_COLORS[tooltip.block.franchise] || "#ccc"
            }`,
            borderRadius: 8,
            padding: "10px 14px",
            pointerEvents: "none",
        ...
        >
          ...
          <div
            style={{
              fontSize: 12,
              color: TEAM_COLORS[tooltip.block.franchise] || "#888",
              marginBottom: 6,
              fontWeight: 700,
            }}
          >
          ...
          <div style={{ fontSize: 12, color: "#666" }}>
            ...
            &nbsp;·&nbsp;{tooltip.block.end - tooltip.block.start}{" "}
            {tooltip.block.end - tooltip.block.start === 1
              ? "season"
              : "seasons"}
          </div>
          <div style={{ fontSize: 13, color: "#333", marginTop: 4 }}>
            <span style={{ color: "#2e7d32", fontWeight: 700 }}>
              {tooltip.block.wins}W
            </span>
            <span style={{ color: "#ccc" }}> – </span>
            <span style={{ color: "#c62828", fontWeight: 700 }}>
              {tooltip.block.losses}L
            </span>
            <span style={{ color: "#888", fontSize: 11 }}>
              ...
              {(
                (tooltip.block.wins /
                  (tooltip.block.wins + tooltip.block.losses)) *
                100
              ).toFixed(1)}
              % win)
            </span>
          ...
          {(() => {
            ...
            return rings > 0 ? (
              ...
              >
                {"🏆".repeat(rings)}
                {rings > 1 ? ` ${rings} championships` : " 1 championship"}
              </div>
            ) : null;
          })()}
        </div>
